...
namespace TimeWaste
{
		...
		m_detour(detour)
	{
		...
		{
			LOG_INFO("Created hook '{}' at 0x{:X}.", m_name, reinterpret_cast<std::uintptr_t>(m_target));
	...
	detour_hook::~detour_hook() noexcept
	{
		...
		LOG_INFO("Removed hook '{}' at 0x{:X}.", m_name, reinterpret_cast<std::uintptr_t>(m_target));
	}
	...
	void detour_hook::enable()
	{
		...
		{
			LOG_INFO("Enabled hook '{}' at 0x{:X}.", m_name, reinterpret_cast<std::uintptr_t>(m_target));
		}
		else
		{
	...
	void detour_hook::disable()
	{
		...
		{
			LOG_INFO("Disabled hook '{}' at 0x{:X}.", m_name, reinterpret_cast<std::uintptr_t>(m_target));
		}
		else
		{
			LOG_INFO("Failed to disable hook '{}' at 0x{:X} (error: {})", m_name, reinterpret_cast<std::uintptr_t>(m_target), MH_StatusToString(status));
		}
	}
