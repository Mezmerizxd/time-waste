...
script_hook::script_hook(rage::joaat_t script_hash,
std::unordered_map<rage::scrNativeHash, rage::scrNativeHandler>
native_replacements)
...
script_hook::~script_hook()
{
	if (m_program)
	{
		...
		{
			auto og_handler = g_pointers->m_get_native_handler(
			g_pointers->m_native_registration_table, hash);
...
void script_hook::ensure()
{
	...
	if (auto program
		= g_pointers->m_script_program_table->find_script(m_script_hash))
	{
		if (program->is_valid())
		{
			hook_instance(program);
			LOG_INFO("Hooked {} script ({}).", program->m_name,
...
void script_hook::hook_instance(rage::scrProgram* program)
{
	...
	for (auto [replacement_hash, replacement_handler] : m_native_replacements)
	{
		...
		auto handler_ptr
		= m_program->get_address_of_native_entrypoint(og_handler);
		if (!handler_ptr)
			continue;

		m_native_handler_ptrs.emplace(
		hash, reinterpret_cast<rage::scrNativeHandler*>(handler_ptr));
...
void script_hook::scrprogram_dtor(rage::scrProgram* this_, bool free_memory)
{
	if (auto it = s_map.find(this_); it != s_map.end())
	{
		...
		auto og_func
		= hook->m_vmt_hook->get_original<decltype(&scrprogram_dtor)>(0);
