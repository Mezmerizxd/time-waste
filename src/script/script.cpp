...
script::script(func_t func, std::optional<std::size_t> stack_size)
	: m_func(func), m_script_fiber(nullptr), m_main_fiber(nullptr)
{
...
void script::tick()
{
	...
	if (!m_wake_time.has_value()
		|| m_wake_time.value() <= std::chrono::high_resolution_clock::now())
	{
...
void script::fiber_func()
{
	__try
	{
		[this]()
		{
			...
			catch (std::exception const& ex)
			{
				...
				LOG_INFO(
				"Script threw an C++ expection! {} : {}", ex_class, ex.what());
			}
			catch (...)
			{
	...
	__except (script_exception_handler(GetExceptionInformation()),
