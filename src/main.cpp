	...
	if (reason == DLL_PROCESS_ATTACH)
	{
		...
		[](PVOID) -> DWORD
		{
			...
			try
			{
				...
				g_script_manager.add_script(
				std::make_unique<script>(&Gui::script_func));
				g_script_manager.add_script(
				std::make_unique<script>(&Menu::script_func));
				g_script_manager.add_script(
				std::make_unique<script>(&Features::script_func));
			...
			catch (std::exception const& ex)
			{
				...
				MessageBoxA(
				nullptr, ex.what(), nullptr, MB_OK | MB_ICONEXCLAMATION);
			}
